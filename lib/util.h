...
public:
    ...
    static typename std::enable_if<false == std::is_convertible<T, std::string>::value,
                                   std::string>::type
    toStr(T const &val) { return std::to_string(val); }
    ...
    static void loading(const unsigned long long size, const unsigned long long index, unsigned short barLength, unsigned short updates)
    {
        ...
        if (size > 0 && index <= size)
        {
            ...
            static short i = -1;
            updates = updates > 99 ? 100 : updates < 1 ? 0
                                                       : updates;
            ...
            if (tokens != i)
            {
                ...
                if (percent != 100 && size > index)
                {
                    ...
                    for (unsigned long j = 0; j < tokens; j++)
                        statusFull += block;
                    for (unsigned long j = 0; j < barLength - (unsigned long)tokens; j++)
                        statusVoid += dotted;
                    if (percent < 33)
                        colors = "red";
                    else if (percent < 66)
    ...
    static double sTod(const std::string str)
    {
        ...
        for (unsigned int i = 0; i < strNumSign.size(); i++)
        {
            ...
            else if ((sns.at(0) != '*' && sns.at(0) != '/') || (i == strNumSign.size() - 1 && (sns.at(0) == '*' || sns.at(0) == '/')))
            {
                ...
                {
                    if (tempNum.at(0) == '*')
                        sum += std::stod(tempNum.substr(1, tempNum.size())) * std::stod(sns);
                    else
                        sum += std::stod(tempNum.substr(1, tempNum.size())) / std::stod(sns);
                }
                else
     ...
     */
    template <typename... T>
    static std::string tableMaker(std::deque<std::deque<std::string>> &allData, std::vector<unsigned long> longest, T const &...vals)
    {
        std::vector<unsigned long> minMax;
        bool title = true;
        std::string color = "";
        std::function<int(double n)> num = [&](int n)
        { minMax.push_back(n>0?(unsigned long)n:0); return 0; };
        std::function<int(bool b)> cond = [&](bool b)
        { title = b; return 0; };
        std::function<int(std::string s)> stri = [&](std::string s)
        { color = s; return 0; };
        using unused = int[];
        (void)unused{0, (getType(vals) == 1 ? num(std::stod(toStr(vals))) : getType(vals) == 2 ? cond(std::stod(toStr(vals)))
                                                                        : getType(vals) == 3   ? stri(toStr(vals))
                                                                                               : 0,
                         0)...};
        return tableMaker(allData, longest, minMax.size() ? minMax[0] : 0, minMax.size() > 1 ? minMax[1] : 0, color, title);
    }
    template <typename... T>
    static std::string tableMaker(std::deque<std::deque<std::string>> &allData, T const &...vals)
    {
        std::vector<unsigned long> minMax;
        bool title = true;
        std::string color = "";
    ...
    static std::string tableMaker(std::deque<std::deque<std::string>> &allData, std::vector<unsigned long> longest, unsigned long minSize, unsigned long maxSize, std::string colors, bool title)
    {
        ...
        const char rowDelim = '-';
        const std::string colorStart = color(colors);
        const std::string colorEnd = colorReset();
        unsigned long size = minSize;
        minSize = minSize < maxSize ? minSize : maxSize;
        maxSize = maxSize > minSize ? maxSize : size;
        ...
        if (minSize + maxSize)
        {
            ...
            if ((double)size - rowSum >= 0)
                for (unsigned long i = 0; i < allData.size(); i++)
                    maxLength[i] = ((double)longest[i] / rowSum) * size;
            else
            {
                unsigned long spaceTaken = 0, bigColumns = allData.size();
                for (unsigned long i = 0; i < allData.size(); i++)
                {
                    if (longest[i] <= (double)size / allData.size())
                        maxLength[i] = longest[i], spaceTaken += longest[i], bigColumns--;
                }
                for (unsigned long i = 0; i < allData.size(); i++)
                {
                    if (longest[i] > (double)size / allData.size())
                        maxLength[i] = (double)(size - spaceTaken) / bigColumns;
                }
            }
            longest = maxLength;
        }
        ...
        {
            if (i == 0)
                rowSize += longest[i] + (1 + columnDelim.size()) * 2;
            else
                rowSize += longest[i] + 2 + columnDelim.size();
        }
        ...
        for (unsigned long long i = 0; i < longestColumn; i++)
        {
            ...
            for (unsigned int j = 0; j < allData.size(); j++)
            {
                ...
                if (str.size() > maxLength[j])
                {
                    ...
                    if (maxLength[j] == 1)
                        str = str.substr(0, 1);
                    else if (maxLength[j] > 1 && maxLength[j] < 5)
                        str = str.substr(0, 1) + excess.substr(0, maxLength[j] - 1);
                    else
                        str = str.substr(0, maxLength[j] - 3) + excess;
                }
                ...
                std::string start = "", end = "";
                if (i == 0 && title)
                    start = color("cyan"), end = colorEnd;
                else if (j == 0 && allData.size() > 1)
                    start = color("yellow"), end = colorEnd;
    ...
    template <typename... T>
    static std::deque<std::string> navOptions(std::vector<std::string> options, const T &...args)
    {
        unsigned long minMax;
        bool print = false;
        std::string color = "";
        std::function<int(double n)> num = [&](int n)
        { minMax = n>0?(unsigned long)n:0; return 0; };
        std::function<int(bool b)> cond = [&](bool b)
        { print = b; return 0; };
        std::function<int(std::string s)> stri = [&](std::string s)
        { color = s; return 0; };
        using unused = int[];
        (void)unused{0, (getType(args) == 1 ? num(std::stod(toStr(args))) : getType(args) == 2 ? cond(std::stod(toStr(args)))
                                                                        : getType(args) == 3   ? stri(toStr(args))
                                                                                               : 0,
                         0)...};
    ...
    static std::deque<std::string> navOptions(std::deque<std::string> options, unsigned long minSize, std::string colors, bool print)
    {
        ...
        std::string cStart = color(colors);
        std::string cEnd = cStart == "" ? "" : colorReset();
